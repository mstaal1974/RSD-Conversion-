...
Tabs:
  ...
  2. Ingest — seed from DB, run TGA ingestion, run linkage engine
  3. Browse — enriched skill statements with occupational context
  4. Export — CSV/OSMT with ANZSCO fields
"""
...
with tab2:
    ...
    for step, label, desc in [
        ...
        ("2", "TGA Ingestion (optional)",
         "Fetch ContentBundle from TGA SOAP API to get packaging rules and ANZSCO classifications. Requires TGA credentials."),
    ...
    st.caption("Reads all unique unit_codes from rsd_skill_records and registers them. Run this first before TGA ingestion.")
    ...
    with tga_col1:
        tga_user_input = st.text_input("TGA username",
                                       value=TGA_USER or "",
                                       type="default")
        tga_pass_input = st.text_input("TGA password",
                                       value=TGA_PASS or "",
                                       type="password")
    with tga_col2:
    ...
    st.caption(
        ...
        "Contact NTRReform@dewr.gov.au to request API access. "
        "Without SOAP credentials, REST mode will register quals/UOCs but cannot extract packaging rules."
    )
    ...
    if st.button("▶ Run TGA ingestion"):
        ...
        tp_list = ([t.strip().upper() for t in tp_filter_input.split(",")
                    if t.strip()]
    ...
    with link_col1:
        ...
        run_asc_match = st.checkbox(
            "Enable Priority 4 (ASC keyword matching)",
            value=True,
    ...
    if st.button("▶ Run linkage engine", type="primary"):
        ...
        try:
            ...
            st.success(
                f"✅ Created **{counts.get('links_created',0)}** occupation links"
            )
        except Exception as e:
            finish_pipeline_run(engine, rid, "failed", error=str(e))
    ...
    st.caption(
        "Download from Jobs and Skills Australia: "
        "jobsandskills.gov.au → Data and research → Australian Skills Classification. "
        "Expected columns: task_id, task_description, anzsco_code, anzsco_title, skill_cluster"
    )
                        ...
                        for _, row in asc_df.iterrows():
                            conn.execute(text("""
                                INSERT INTO asc_specialist_tasks
                                    (task_id, task_description, anzsco_code,
                                     anzsco_title, skill_cluster, technology_level)
                                VALUES (:tid, :tdesc, :ac, :at, :sc, :tl)
                            ...
                            """), {
                                ...
                                "sc":    str(row.get("skill_cluster","")),
                                "tl":    str(row.get("technology_level","")),
                            })
...
with tab3:
    ...
    if df_linked.empty:
        ...
        # Render first 50 rows as styled cards
        show_df = df_linked.head(50)
        for _, row in show_df.iterrows():
            ...
            st.markdown(
                ...
                f"<span style='color:#5c6bc0;font-size:0.75rem'>"
                f"{anzsco} {anzsco_t}</span>"
                f"{badge}"
                f"<span style='color:#37474f;font-size:0.72rem'>{src}</span>"
                f"</div>"
                f"<div style='font-size:0.87rem;color:#cfd8dc'>{stmt}</div>"
                 f"{ '<div style=\'font-size:0.72rem;color:#37474f;margin-top:4px\'>' + major + (' · ' + ind if ind else '') + '</div>' if major else '' }"
                f"</div>",
                unsafe_allow_html=True,
            )

        if len(df_linked) > 50:
            st.caption(f"Showing 50 of {len(df_linked)} — use export for full set")
...
# ─────────────────────────────────────────────────────────────────────────────
# TAB 4 — VC RECORD  (W3C Verifiable Credential aligned JSON)
# ─────────────────────────────────────────────────────────────────────────────
with tab4:
    import json
    from core.rsd_record import build_uoc_record, records_to_jsonl, anzsco_uri, aqf_to_skill_label

    st.markdown('<div class="section-hdr">W3C Verifiable Credential — Fully linked UOC record</div>',
                unsafe_allow_html=True)
    st.caption(
    ...
    with col_vc1:
        ...
        if not uoc_list:
            ...
            uoc_options = {f"{code} — {title[:50]}": code
                          for code, title in uoc_list}
            sel_uoc_label = st.selectbox("Select UOC", list(uoc_options.keys()),
                                          key="vc_uoc_sel")
            sel_uoc_code  = uoc_options[sel_uoc_label]
            include_all   = st.checkbox("Include all occupation links",
                                         value=False,
        ...
        # ── Evidence management (Ref 3) ───────────────────────────────────────
        st.subheader("Evidence linking (Ref 3)")
        st.caption(
            "Link a Blocksure/IPFS hash to a specific skill statement. "
            "The hash is stored in rsd_skill_records.evidence_hash and "
            "surfaced in the VC record."
        )
        ...
        ev_hash    = st.text_input("Evidence hash (IPFS CID or Blocksure ID)",
        ...
        st.caption("Auto-generate stable element IDs (e.g. MSL975058-E01) for all records.")
        ...
        if st.button("▶ Generate element IDs for all records", key="gen_eids"):
            ...
            # Build element IDs grouped by unit
            from collections import defaultdict
            unit_idx: dict = defaultdict(int)
            with engine.begin() as conn:
                for row in rows:
                    rec_id, uc, elem, _ = row
                    eid = build_element_id(uc or "", elem or "",
                                           unit_idx[uc])
                    unit_idx[uc] += 1
                    conn.execute(text(
    ...
    with col_vc2:
        ...
        if not record:
            ...
            mini_card(s3, "✓" if not imp.get("is_imported") else "↗ IMPORTED",
                      "Origin")
            mini_card(s4, f"{ta.get('confidence',0):.2f}", "Conf")

            st.markdown("")

            # ── Taxonomic alignment display (Ref 1) ────────────────────────────
            if ta:
            ...
            for stmt in stmts[:5]:
                ...
                st.markdown(
                    ...
                    f'Evidence: {ev.get("type","")} · Hash: {ev.get("hash","")[:40]}…' +
                    f'</div></div>',
                    unsafe_allow_html=True,
                )
            if len(stmts) > 5:
                st.caption(f"… and {len(stmts)-5} more statements. Download JSON for full record.")
            ...
            dl1.download_button(
                "⬇ Download VC JSON",
                json_str.encode(),
                f"{uoc_c}_vc_record.json",
                "application/json",
                use_container_width=True,
            )

            # JSONL batch for selected TP
            tp_for_batch = record.get("tp_code","")
            if tp_for_batch:
                if st.button(f"⬇ Build JSONL batch for all {tp_for_batch} UOCs",
                             key="vc_batch"):
                    with engine.connect() as conn:
                    ...
                    dl2.download_button(
                        f"⬇ {tp_for_batch} JSONL batch ({len(batch_codes)} UOCs)",
                        jsonl.encode(),
                        f"{tp_for_batch}_vc_batch.jsonl",
...
with tab5:
    ...
    if export_df.empty:
        ...
        st.info(f"**{n_exp:,}** enriched skill statements ready for export")
        ...
        c1.download_button(
            ...
            "rsd_enriched_occupational.csv",
            "text/csv",
            use_container_width=True,
            help="All skill statements with ANZSCO/ASCED fields"
        )

        # ── OSMT-compatible with ANZSCO ───────────────────────────────────────
        osmt_cols = {
            "unit_code":    "Standards",
            "unit_title":   "Category",
        ...
        c2.download_button(
            ...
            "osmt_with_anzsco.csv",
            "text/csv",
            use_container_width=True,
            help="OSMT import format enriched with ANZSCO fields"
        )

        # ── Summary by ANZSCO occupation ──────────────────────────────────────
        summary = (
            export_df[export_df["anzsco_code"].notna() & (export_df["anzsco_code"] != "")]
            .groupby(["anzsco_code","anzsco_title","anzsco_major_group"])
            .agg(
                uoc_count=("unit_code","nunique"),
                stmt_count=("skill_statement","count"),
                avg_confidence=("confidence","mean"),
                training_packages=("unit_code",
        ...
        c3.download_button(
            ...
            "text/csv",
            use_container_width=True,
            help="One row per ANZSCO code — how many UOCs and statements map to each occupation"
        )

        st.divider()

        # ── Preview occupation summary ────────────────────────────────────────
