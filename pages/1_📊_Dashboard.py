...
DB dashboard — overview stats, run history, skill statement browser,
inline editing, category management, and data quality checks.
"""
...
def get_engine(url: str):
    from core.db import get_engine as _get
    return _get(url)
    ...
    with engine.connect() as conn:
        res = conn.execute(text("""
            SELECT
                ...
                r.model,
                r.provider,
                COUNT(s.record_id)                          AS total_records,
    ...
    with engine.connect() as conn:
        stats = conn.execute(text("""
            SELECT
                ...
                COUNT(s.record_id)                              AS total_skills,
...
if not stats or stats.get("total_skills") in (None, 0):
    st.info("No skill records in the database yet. Run a batch to populate.")
else:
...
if runs_df.empty:
    ...
    display_runs["qa_rate"] = display_runs.apply(
        ...
        if r.get("total_records") else "—",
        axis=1,
    )

    show_cols = [c for c in [
        "run_id", "created_at_utc", "status", "source_filename",
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "created_at_utc": st.column_config.DatetimeColumn("Created", format="DD/MM/YY HH:mm"),
...
# ── Skill statement browser + inline editor ────────────────────────────────────
...
if not skills_df.empty:
    ...
    if search:
        mask = skills_df["skill_statement"].str.contains(search, case=False, na=False)
    ...
    edit_cols = [c for c in [
        ...
        "skill_statement", "keywords_semicolon",
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "keywords_semicolon": st.column_config.TextColumn("Keywords"),
            ...
            for i, row in edited.iterrows():
                ...
                conn.execute(
                    ...
                    """),
                    {
                        ...
                        "kw":    row.get("keywords_semicolon", ""),
                        "rid":   original_row["run_id"],
                        "recid": original_row["record_id"],
                    },
                )
                updated += 1
...
if not all_skills.empty:
    ...
    # Check 1: Empty skill statements
    ...
    # Check 3: Word count out of range
    ...
    # Check 4: Missing keywords
    kw_col = "keywords_semicolon" if "keywords_semicolon" in all_skills.columns else "keywords"
    if kw_col in all_skills.columns:
        no_kw = all_skills[
            all_skills[kw_col].isna() | (all_skills[kw_col].str.strip() == "")
        ]
        if len(no_kw):
            issues.append(f"ℹ️ **{len(no_kw)}** statements have no keywords")

    # Check 5: Duplicate skill statements
...
if not all_skills.empty and kw_col in all_skills.columns:
    all_kws = (
        all_skills[kw_col]
        .dropna()
        .str.split(";")
    ...
    if len(all_kws):
        ...
        st.info("No keywords found. Enable keyword generation when running batches.")
else:
